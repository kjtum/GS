...
def draw_state_with_proposals(matching, proposals, men_prefs, women_prefs):
    ...
    for i, m in enumerate(MEN):
        ...
        ax.text(x_men - 0.12, y, f"({3 - men_prefs[m].index(st.session_state.engaged[m]) if m in st.session_state.engaged else 0}) {m}", fontsize=5, va='center', ha='right')
    for i, w in enumerate(WOMEN):
        ...
        ax.text(x_women + 0.12, y, f"({3 - women_prefs[w].index(st.session_state.engaged_inv[w]) if w in st.session_state.engaged_inv else 0}) {w}", fontsize=5, va='center', ha='left')
...
with col1:
    st.markdown(
        f"<div style='font-size:20px; line-height:1.0;'>満足度: 合計={total}, 男性側={ms}, 女性側={ws}, 差={diff}, 最小満足度={min_satis}</div>",
        unsafe_allow_html=True
    )
    if len(st.session_state.free_women) == 0:
        st.markdown(
            "<div style='font-size:25px; line-height:1.0;'><br>🎉 安定マッチング完了</div>",
            unsafe_allow_html=True
        )
