...
def draw_state_with_proposals(matching, proposals, men_prefs, women_prefs):
    ...
    for i, m in enumerate(MEN):
        ...
        ax.text(x_men - 0.1, y, f"(0) {m}", fontsize=2.5, va='center', ha='right')
    for i, w in enumerate(WOMEN):
        ...
        ax.text(x_women + 0.1, y, f"(0) {w}", fontsize=2.5, va='center', ha='left')
...
total, man_score, woman_score, diff, max_score = calculate_dissatisfaction(
